  ...
  public partial class MainWindow : Window {
    ...
    private static async Task DevUpdateCheck() {
      try {
        ...
        string? newVersion = await Utils.CheckDevUpdate(VERSION);
        if (newVersion != null)
          new UpdateNotifier(VERSION, newVersion, $"https://github.com/RagingLightning/TSW3-LM/releases/tag/v{newVersion}").ShowDialog();
      }
      catch (WebException e) {
    ...
    internal void UpdateLibraryGui(bool tsw3 = true) {
      if (tsw3) {
        ...
        if (string.IsNullOrEmpty(Config.LibraryPath)) return;
    ...
    private void btnDonate_Click(object sender, RoutedEventArgs e) {
      Process.Start(new ProcessStartInfo() { FileName = "https://www.paypal.com/donate/?hosted_button_id=DM5BH83828KCL", UseShellExecute = true });
    }
  }
